...
In this part, we provide the proof for Theorem \ref{theorem:neural-bo_main}. The main lemmas are from Lemma \ref{lemma:neural-bo_bound_hil} to Lemma \ref{lemma:neural-bo_min_sigma}. Some main lemmas require auxiliary lemmas to complete their proofs. These auxiliary lemmas are provided in Section \ref{section:neural-bo_aux_appendix} and later proved in Section \ref{section:neural-bo_aux_proofs}. 
...
\begin{subdefinition}
\label{def:neural-bo_linear_kernelized_terms}
...
\\
        & \leq \sqrt{2}B \sigma_t(\mathbf{x}) + \sigma_t(\mathbf{x})\frac{R}{\sqrt{\lambda}} \sqrt{2 \log(\frac{1}{\alpha})} \label{ieqn:neural-bo_confidence_interval},
\end{align}
where the first inequality uses triangle inequality and the fact that $\mathbf{r}_{t-1}= \mathbf{f}_{t-1} + \boldsymbol{\epsilon}_{t-1}$. The second inequality is from the reproducing property of function relying on RKHS, and the fourth equality is from the verification noted in Definition  \ref{def:neural-bo_linear_kernelized_terms}. The last inequality directly uses the results from Lemma \ref{lemma:noise_affeted_bound} and Lemma \ref{lemma:neural-bo_RKHS_expression}. We have a more compact form of the inequality \eqref{ieqn:neural-bo_confidence_interval} as: 
...
Then, by combining this bound with Lemma \ref{lemma:neural-bo_NN_vs_linear}, we have:
...
\noindent For any step $t$, we consider how the standard deviation of the estimates for each point is, in comparison with the standard deviation for the optimal value.
...
\begin{equation*}
\label{def:neural-bo_saturated_set}
...
\label{lemma:neural-bo_unsaturated_points}
...
This proof inherits the proof of Lemma 4.6 in \citet{zhang2021neural}, and by using the result of unsaturated point $\mathbf{x}_t$ in Lemma \ref{lemma:neural-bo_unsaturated_points}, along with $\lvert f(x)  \rvert = \lvert \langle f, k(x,\cdot) \rangle \rvert \leq B$ instead of $\lvert f(x) \rvert  \leq 1$ as in \citet{zhang2021neural}, we have the following result:
\begin{equation*}
\begin{split}
...
\end{split}
\end{equation*}
where $\{C_{\epsilon,i}\}_{i=1}^3$ are positive constants. \textbf{We provide the proof for Lemma \ref{lemma:neural-bo_NN_vs_linear} in Section \ref{section:neural-bo_NN_vs_linear_proof}}.
\end{auxlemma}

\begin{auxlemma}
\label{lemma:noise_affeted_bound}
Let $\alpha \in (0,1)$. Recall that matrix $\mathbf{U}_{t-1}$ is defined in Algorithm \ref{alg:Neural-BO}, $\mathbf{G}_{t-1}$ is defined in Definition \ref{def:neural-bo_linear_kernelized_terms} and $\epsilon_{t-1}$ is i.i.d sub-Gaussian  observation noises up to optimization iteration $t-1$, with parameter $R$. Then with probability  $1-\alpha$, we have
...
where $\gamma_t$ is maximum information gain associated with the kernel $k_\textup{NTK}$. We provided the proof of Lemma \ref{lemma:neural-bo_log_det_Kt_bound} in \ref{section:neural-bo_log_det_Kt_bound_proof}.
\end{auxlemma}

\begin{auxlemma}[Lemma D.2, \citep{kassraie2022neural}] 
\label{lemma:neural-bo_RKHS_expression}
Let $\alpha \in (0,1)$. Under Assumption \ref{assumption:neural-bo_sufficient_exploration}, if the network width $m$ satisfies  $m \geq C T^6L^6 \log(TL/\alpha)$ and $f$ be a member of $\mathcal{H}_{k_\text{NTK}}$ with bounded RKHS norm $\norm{f}_{k_\text{NTK}} \leq B$, then with probability at least $1-\alpha$, there exists $\mathbf{w} \in \mathbb{R}^p$ such that 
\[ f(\mathbf{x}) = \langle \mathbf{g}(\mathbf{x}; \boldsymbol{\theta}_0), \mathbf{w} \rangle, \norm{\mathbf{w}}_2 \leq \sqrt{\frac{2}{m}}B.\]
\end{auxlemma}

\begin{auxlemma}
\label{lemma:neural-bo_min_B_vs_cov_norm}
...
\]
\end{auxlemma}
\noindent Section \ref{section:neural-bo_min_B_vs_cov_norm_proof} provided the proof for Lemma \ref{lemma:neural-bo_min_B_vs_cov_norm}. 
\subsection{Proof of Auxiliary Lemmas}
\label{section:neural-bo_aux_proofs}
\subsubsection{Proof of Lemma \ref{lemma:neural-bo_NN_vs_linear}}
\label{section:neural-bo_NN_vs_linear_proof}
...
\begin{enumerate}
    ...
    \label{res:neural-bo_true_f_vs_linear}
...
\begin{auxlemma}
\label{lemma:neural-bo_lin_vs_regresion}
...
Set $\tau=2\sqrt{t/m\lambda}$, it can be verified that $\tau$ satisfies condition in \ref{lemma:NTK_related_bounds}. Therefore,  by inequality \eqref{res:neural-bo_true_f_vs_linear} in Lemma \ref{lemma:NTK_related_bounds}, and with the initialization of the network $f(\mathbf{x};\boldsymbol{\theta}_0) = 0$, there exists a constant $C_{\epsilon,1}$ such that
...
\end{align*}
where the first inequality is from (\ref{res:neural-bo_grad_bound}) and Lemma \ref{lemma:neural-bo_lin_vs_regresion}, with $C_{\epsilon,2} = \bar C_{\epsilon,1}$ and  $C_{\epsilon,3} = \bar C_{\epsilon,1} \bar C_{\epsilon,2}$.
Finally, we have 
\begin{align*}
...
\label{lemma:neural-bo_noise_affeted_bound_proof}
...
\\
    & = \mathbb \exp \left( \frac{R^2\norm{\mathbf{Z}_{t-1}(\mathbf{x})}^2_2}{2} \right) \label{eqn:neural-bo_noise_effect}, 
\end{align}
where the second equation is the consequence of independence of $\zeta_i(\mathbf{x})\boldsymbol{\epsilon}_i$, which directly utilizes our i.i.d noise assumption which is mentioned in Assumption \ref{assumption:neural-bo_iid_noise}. The first inequality holds by the concentration property of the sub-Gaussian noise random variable with parameter $R$. Then we bound:  
\begin{align}
...
\\
        & \leq \frac{1}{\lambda} \sigma_t^2(\mathbf{x}), \label{eqn:neural-bo_bound_Z}
\end{align}
where the second equality is from definition of $\mathbf{Z}_{t-1}$ and the third inequality is from the note of Definition \ref{def:neural-bo_linear_kernelized_terms}.  The inequality is from the fact that  $\frac{\lambda}{m} \norm{\mathbf{g}(\mathbf{x}; \boldsymbol{\theta}_0) \mathbf{U}^{-2}_{t-1}}^2 \ge 0, \forall \mathbf{x},t$. Replace inequality \eqref{eqn:neural-bo_bound_Z} to Eqn. \eqref{eqn:neural-bo_noise_effect} then we have 
...
\label{section:neural-bo_log_det_Kt_bound_proof}
...
\end{split}
\end{equation*}
where the first equality is from the definition of $\mathbf{K}_t$ in Definition \ref{def:neural-bo_linear_kernelized_terms}, the first inequality is from the convexity of $\log \det(\cdot)$ function,
...
\subsubsection{Proof of Lemma \ref{lemma:neural-bo_min_B_vs_cov_norm}}
\label{section:neural-bo_min_B_vs_cov_norm_proof}
...
\begin{split}
  ...
  & = \det(\mathbf{V}_{t-1}) \left(1+\norm{\mathbf{v}_{t-1}}^2_{\mathbf{V}_{t-1}^{-1}} \right) \\
  & = \det (\lambda \mathbf{I}) \prod_{t=1}^T \left(1+\norm{\mathbf{v}_{t-1}}^2_{\mathbf{V}_{t-1}^{-1}} \right),
\end{split}
\end{equation*}
