...
\section{Proof of Theoretical Results in Chapter \ref{chap:pinn-bo}}
\label{section:pinn-bo_appendix}

In this section, we present the detailed proof of Lemma \ref{lemma:pinn-bo_PINN_mean_cov}, Lemma \ref{lemma:pinn-bo_confidence_bound} and Theorem \ref{theorem:pinn-bo_main} in Chapter \ref{chap:pinn-bo}.
 As these theoretical results require auxiliary lemmas to complete their proofs, we provide auxiliary lemmas in Section \ref{section:pinn-bo_aux_appendix} and later prove in Section \ref{section:pinn-bo_aux_proofs}. 
\subsection{Proof of Lemma \ref{lemma:pinn-bo_PINN_mean_cov}}
 Before going to the proof, we repeat Lemma \ref{lemma:pinn-bo_PINN_mean_cov} here:
\PinnMeanCov*
...
\subsection{Proof of Theorem \ref{theorem:pinn-bo_main}}
Before going to the proof, we repeat Theorem \ref{theorem:pinn-bo_main} here:
\PINNBORegretBound*


% \noindent \textbf{To prove Theorem \ref{theorem:pinn-bo_main}, we start by proving the following lemmas:}

\noindent \textbf{Now we are ready to prove the Theorem \ref{theorem:pinn-bo_main}}.

\begin{proof}[Proof of Theorem \ref{theorem:pinn-bo_main}]
...
\label{section:pinn-bo_aux_appendix}
...
\label{lemma:pinn-bo_rkhs_derivative}
    Let $s \in \mathbb{N}$ and $k \colon \mathcal{X} \times \mathcal{X} \rightarrow \mathbb{R}$ be a Mercer kernel such that $k \in C^{2s} (\mathcal{X} \times \mathcal{X})$. Denote $D^\alpha f(\mathbf{x}), \alpha \in \mathbb{Z}^n_+$ is the partial derivative of $f$ at $\mathbf{x} \in \mathcal{X}$ (if it exists) as:
\begin{align*}
...
Let $\{\mathcal{F}_t\}_{t=0}^\infty$ be a filtration. Let $\{\eta_t\}_{t=0}^\infty$ be a real-valued stochastic process such that $\eta_t$ is $\mathcal{F}_t$-measurable and $\eta_t$ is conditionally $R$-sub-Gaussian for some $R \ge 0$. Let $\{X_t\}_{t=0}^\infty$ be an $\mathbb{R}^d$-valued stochastic process such that $X_t$ is $\mathcal{F}_{t-1}$-measurable. Assume that $\mathbf{V}$
...
\begin{align*}
    \frac{1}{2}\log \det(\frac{\boldsymbol{\Phi}_t^\top \boldsymbol{\Phi}_t}{\lambda_1} + \frac{\boldsymbol{\Omega}_r^\top \boldsymbol{\Omega}_r}{\lambda_2} + \mathbf{I}) & \le  \frac{1}{2}\log\det (\frac{\mathbf{K}_{uu}}{\lambda_1} +\mathbf{I})+\frac{N_r L^2}{2(1+ \rho_{min}(\mathbf{K}_{uu})/\lambda_1)}
    \\
...
\label{lemma:pinn-bo_var_sum_bound}
    Let $\mathbf{x}_1, \mathbf{x}_2,\dots, \mathbf{x}_T$ be the points selected by the Algorithms \ref{alg:PINN-BO} in Chapter \ref{chap:pinn-bo}, then:
...
\label{section:pinn-bo_aux_proofs}
...
\begin{align*}
    ...
    \end{align*}
The fourth equality leveraged the property that a matrix in the form of $\mathbf{I} + \mathbf{v}\mathbf{v}^\top$ has an eigenvalue of $1 + \lVert \mathbf{v} \rVert^2$, associated with the eigenvector $\mathbf{v}$, while all other eigenvalues are equal to one. The 7th equality utilized Weinstein-Aronszajn identity, which indicates that $\det(\frac{\boldsymbol{\Phi}_t^\top \boldsymbol{\Phi}_t}{\lambda_1} + \mathbf{I}) = \det(\frac{\mathbf{K}_{uu}}{\lambda_1} + \mathbf{I})$. The first inequality is from $\log(1+a) \le a$ and $\gamma_t := \max_{\mathcal{A} \subset \mathcal{D}: \lvert \mathcal{A} \rvert=t} I(\mathbf{Y}_\mathcal{A}; f_\mathcal{A}) \ge \frac{1}{2}\log \det(\frac{\mathbf{K}_{uu}}{\lambda_1} + \mathbf{I})$, while the second inequality uses the min-max theorem for Hermitian matrices. The last equality used properties of eigenvalues of shifted matrices. 




\subsubsection{Proof of Lemma \ref{lemma:pinn-bo_var_sum_bound}}
Conditioned on observations vector $\mathbf{Y}_{1:s-1}$  observed at points $\mathcal{A}_{s-1} \subset \mathcal{D}$, and the PDE vector $\mathbf{U}_r = \mathbf{U}_{1:N_r}$, the reward $y_s$ at round $s$ observed at
