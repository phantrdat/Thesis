...
\label{section:neural-cbo_further_results}
\paragraph{Justification for the metric used in Section \ref{section:neural-cbo_synthetic}, Chapter \ref{chap:neural-cbo}:}

In Section \ref{section:neural-cbo_synthetic}, Chapter \ref{chap:neural-cbo}, we report the (Log10 of) the Best Positive Regret plus Violation. To justify our choice of metrics, we consider the definitions of positive regret and violation in the context of a minimization problem and emphasize the following:
...
To provide a more comprehensive evaluation of each method, we report results using three metrics: \textbf{Cumulative Positive Regret}, \textbf{Cumulative Violation}, and \textbf{Best Positive Regret for Feasible Points}. These metrics are illustrated in Figure~\ref{fig:neural-cbo_synthetic_cumu_regret}, Figure~\ref{fig:neural-cbo_synthetic_cumu_violation}, and Figure~\ref{fig:neural-cbo_synthetic_best_feasible_minimum}, respectively. As shown, Neural-CBO consistently achieves strong performance across all metrics, demonstrating both rapid convergence to the optimum and effective constraint satisfaction. In particular, for the best positive regret among feasible points, Neural-CBO reliably identifies feasible minima. Note that the starting points in Figure~\ref{fig:neural-cbo_synthetic_best_feasible_minimum} differ between methods, as each algorithm may encounter the feasible region at different iterations.

\begin{figure*}[h]
    \centering
   \includegraphics[width=\linewidth]{figures/Neural-CBO/Branin-Simionescu-GasTransmission-Ackley-Hartmann-SpeedReducer_regret.pdf}
   % \vspace{0.15in}
    ...
    \label{fig:neural-cbo_synthetic_cumu_regret}
\end{figure*}


\begin{figure*}[h]
    \centering
   \includegraphics[width=\linewidth]{figures/Neural-CBO/Branin-Simionescu-GasTransmission-Ackley-Hartmann-SpeedReducer_violation.pdf}
   % \vspace{0.15in}
    \caption{The plots show Cumulative Violation up to step $t$, which is $\sum_{t=1}^T [f(\mathbf{x}_t) - f^*]^+$, comparing our proposed algorithm and four baselines. The dimension of each objective function is shown in the parenthesis. The left group is four synthetic functions introduced in Section \ref{section:neural-cbo_synthetic}, while the right group is the optimization results of Gas Transmission Compressor Design and Speed Reducer Design, described in Section  \ref{section:neural-cbo_gas} and \ref{section:neural-cbo_speed}.}
    \label{fig:neural-cbo_synthetic_cumu_violation}
\end{figure*}

\begin{figure*}[h]
    \centering
   \includegraphics[width=\linewidth]{figures/Neural-CBO/Branin-Simionescu-GasTransmission-Ackley-Hartmann-SpeedReducer_best_feasible_minimum.pdf}
   % \vspace{0.15in}
    \caption{The plots show Best Positive Regret for Feasible Points up to step $t$, which is $\sum_{t=1}^T [f(\mathbf{x}_t) - f^*]^+$, comparing our proposed algorithm and four baselines. The dimension of each objective function is shown in the parenthesis. The left group is four synthetic functions introduced in Section \ref{section:neural-cbo_synthetic}, while the right group is the optimization results of Gas Transmission Compressor Design and Speed Reducer Design, described in Section  \ref{section:neural-cbo_gas} and \ref{section:neural-cbo_speed}.}
    \label{fig:neural-cbo_synthetic_best_feasible_minimum}
\end{figure*}


\paragraph{Wall-clock Time Comparison:}

We provide the wall-clock running time of our algorithm and the considered baselines in the following table. We revisit six optimization tasks Chapter \ref{chap:neural-cbo} but increase the number of iterations for Hartmann ($D = 6$) and Speed Reducer ($D = 7$) to $n_{\text{iter}} = 1000$, to demonstrate the performance of neural networks when run for a large number of iterations. While \ac{gp}-based methods appear more efficient in terms of runtime with a small number of iterations, this advantage diminishes as the number of iterations increases - an often necessary condition in high-dimensional or challenging problems to approach the global optimum. Due to their cubic time complexity with respect to the number of data points, \acf{gp} exhibit significant scalability limitations. In contrast, neural networks enjoy nearly linear scaling in runtime, highlighting their suitability and efficiency for higher number of iterations.
\begin{table}[ht!]
\centering
\resizebox{\textwidth}{!}{%
...
\section{Proof of Theoretical Results in Chapter \ref{chap:neural-cbo}}
\label{section:neural-cbo_appendix}

In this section, we will provide the proof of Lemma \ref{lemma:neural-cbo_confidence_bound} and Theorem \ref{theorem:neural-cbo_main} in Chapter \ref{chap:neural-cbo}. As these theoretical results require auxiliary lemmas to complete their proofs, we provide auxiliary lemmas in Section \ref{section:neural-cbo_aux_appendix} and later prove in Section \ref{section:neural-cbo_aux_proofs}. 

Before going to the proof, we briefly review existing terms and introduce new notations for convenience.
Remind that $\mathbf{g}_{a}(\mathbf{x}; \mathbf{W}) = \nabla_{\mathbf{W}}a(\mathbf{x}; \mathbf{W})$. Therefore, $\mathbf{g}_{a}(\mathbf{x}; \mathbf{W}_0)$ and $\mathbf{g}_{a}(\mathbf{x}; \mathbf{W}_t)$ will be the gradients of the neural network $a(\mathbf{x}; \mathbf{W})$ (using to model an \textit{arbitrary} function $f_a$, defined in Eqn. \eqref{eqn:neural-cbo_fcn}) at initialization and at iteration $t$, respectively.  Further, let us define terms as follows:

\begin{align}  
\mathbf{G}_{a,t-1} & = [\mathbf{g}_{a}(\mathbf{x}_1; \mathbf{W}_0),\dots, \mathbf{g}_{a}(\mathbf{x}_{t-1}; \mathbf{W}_0)],   \label{def:neural_cbo_linear_kernelized_terms_1}
\\
\Bar{\mathbf{G}}_{a,t-1} & = [\mathbf{g}_{a}(\mathbf{x}_1; \mathbf{W}_{t-1}),\dots, \mathbf{g}_{a}(\mathbf{x}_{t-1}; \mathbf{W}_{t-1})],  \label{def:neural_cbo_linear_kernelized_terms_2}
\\
\mathbf{U}_{a,t-1} &=  \mathbf{I} + \mathbf{G}_{a,t-1} \mathbf{G}_{a,t-1}^\top, \label{def:neural_cbo_linear_kernelized_terms_3}
\\
\mathbf{F}_{a,t-1} &= [f_a(\mathbf{x}_1), \dots, f_a(\mathbf{x}_{t-1})].  \label{def:neural_cbo_linear_kernelized_terms_4}
\end{align}
Further, it can be verified that 
\begin{align}
    \label{def:neural_cbo_H0}
    \mathbf{H}_0 = \mathbf{G}_{a,t-1} ^\top\mathbf{G}_{a,t-1}, 
\end{align}
where $\mathbf{H}_0$ is the NTK matrix at initialization defined in Definition \ref{def:neural-cbo_ntk}.  Now we are ready to bound Lemma \ref{lemma:neural-cbo_confidence_bound}.
\subsection{Proof of Lemma \ref{lemma:neural-cbo_confidence_bound}}
First, we repeat Lemma \ref{lemma:neural-cbo_confidence_bound} here:
\NeuralCBOConfidenceBound*

\begin{proof}[Proof of Lemma \ref{lemma:neural-cbo_confidence_bound}]
To prove Lemma \ref{lemma:neural-cbo_confidence_bound}, we analyze the left-hand side as follows:
\begin{align*}
...
First, following Assumption \ref{assumption:neural-cbo_rkhs} in the Chapter \ref{chap:neural-cbo}, we assume that $f_a$ is in RKHS $\mathcal{H}_{k_a}$ with \ac{ntk} kernel $k_a$, and $\mathbf{g}_a(\mathbf{x}; \mathbf{W}_0)$  can be considered as finite approximation of $\varphi(\cdot)$, the feature map of the NTK from $\mathbb{R}^d \rightarrow \mathcal{H}_{k_a}$. From Lemma \ref{lemma:neural-bo_RKHS_expression}, there exists $f_a^* \in \mathbb{R}^p$ such that $f_a(\mathbf{x}) = \langle \mathbf{g}_a(\mathbf{x}; \mathbf{W}_0), f_a^* \rangle = \mathbf{g_a}(\mathbf{x}; \mathbf{W}_0)^\top f_a^*$. 
...
\NeuralCBOTheoremMain*

\begin{proof} [Proof of Theorem \ref{theorem:neural-cbo_main}]
...
\label{section:neural-cbo_aux_appendix}
\begin{auxlemma}[Lemma 10, \citet{tran2022regret}]
...
\begin{auxlemma}[Lemma 14, \citet{tran2022regret}]
...
The following lemma gives the concentration of NTK at the initialization of the neural network introduced in Eqn. \eqref{eqn:neural-cbo_fcn}.
\begin{auxlemma}[Theorem 1, \citet{xu2024overparametrized}]
...
\begin{auxlemma}[Proposition 9, \citet{xu2024overparametrized}] Consider the neural network introduced in Eqn. \eqref{eqn:neural-cbo_fcn} and assume that the condition \ref{condition:neural-cbo_network_width} holds. With probability $1 - \exp (\Omega(C^{-L} m^{1/36}))$, for any sample path $\{ \mathbf{x}_s, y_s\}_{s=0}^{T-1}$, all $t\le T$, we have
\begin{align*}
\begin{split}
...
\end{auxlemma}
The next lemma shows the reproducing property of function $f_a$ being assumed to belong to RKHS induced by NTK kernel $k_a$ of the neural network $a(\mathbf{x}; \mathbf{W})$ introduced in Eqn. \eqref{eqn:neural-cbo_fcn}.
\begin{auxlemma}
    \label{lemma:neural_cbo_RKHS_expression}
    Let $f_a$ be a member of $\mathcal{H}_{k_a}$ with bounded RKHS norm $\norm{f_a}_{\mathcal{H}_{k_a}} \le B_a$. Assume that the network width of the model used to estimate function $f_a (\cdot)$ satisfies the Condition \ref{condition:neural-cbo_network_width}, 
...
\begin{auxlemma}[Lemma 12, \citet{xu2024overparametrized}]
\label{lemma:neural_cbo_nn_sensitivity_bound}
Consider the neural network introduced in Eqn. \eqref{eqn:neural-cbo_fcn} and assume that the condition \ref{condition:neural-cbo_network_width} holds. With probability $1 - \exp (\Omega( C_1^{-L+l} m^{1/36}))$, for layer $l$ and any sample path $\{ \mathbf{x}_s, y_s\}_{s=0}^{T-1}$, with all $t\le T$, we have:
\begin{align*}
...
\begin{auxlemma}[Lemma 10, \citet{xu2024overparametrized}]
\label{lemma:neural_cbo_weights_and_output_bounds}
Consider the neural network introduced in Eqn. \eqref{eqn:neural-cbo_fcn} and assume that the condition \ref{condition:neural-cbo_network_width} holds. Setting the step size at training step $t$ as $\alpha_t \le \frac{\nu}{(T+1)^2}$, then with probability $1 - \exp (\Omega( C^{-L} m^{1/36}))$, for any sample path $\{ \mathbf{x}_s, y_s\}_{s=0}^{T-1}$, all $t\le T$, we have:
\begin{align*}
...
\begin{auxlemma} Let $a(\mathbf{x}; \mathbf{W})$ is the neural network defined in Eqn. \eqref{eqn:neural-cbo_fcn}. Then, with probability $1 - \exp (\Omega(C^{-L} m^{1/36}))$, we have: 
...
conditioned on $\mathcal{F}_{a, t-1}$. Recall $\mathbf{H}_{0}$ defined in Eqn.  \eqref{def:neural_cbo_H0}. For a given
...
Let $\delta \in (0,1)$. If the network width $m$ satisfies  Condition \ref{condition:neural-cbo_network_width}, then with probability at least $1-\delta$, the following holds for every $t \in [T]$:
...
Suppose the width of the neural network m satisfies Condition \ref{condition:neural-cbo_network_width}. Then 
\begin{equation*}
...
\label{section:neural-cbo_aux_proofs}
\subsubsection{Proof of Lemma \ref{lemma:neural_cbo_RKHS_expression}}
    ...
    \end{align*}
    where $\lambda_0$ is a constant which is independent of $m$. The second inequality is from the choice of $m$ in Condition \ref{condition:neural-cbo_network_width}. 
    Then, we have:
    \begin{align*}
...
\noindent Using the technique in the proof of Lemma 8.2 in \citep{allen2019convergence},  there exist diagonal matrices $\widehat{\mathbf{D}}^{(l)} (\mathbf{x}) = \mathbf{D}_{t-1}^{(l)}(\mathbf{x}) - \mathbf{D}_{0}^{(l)}(\mathbf{x})   \in \mathbb{R}^{m \times m}, \forall 1\le l \le L$ with entries in
$[-1,1]$ such that:
\begin{align*}
...
\end{align*} 
The first inequality uses triangle inequality while the second inequality is from Lemma \ref{lemma:neural_cbo_nn_sensitivity_bound} and Lemma  \ref{lemma:neural_cbo_weights_and_output_bounds}.
\subsubsection{Proof of Lemma \ref{lemma:neural_cbo_lin_approx_vs_theoretical_regression_sol}}

Using the model update formula given in Eqn. \eqref{eqn:neural-cbo_train_NN}, we have 
\begin{align*}
...
\end{split}
\end{equation*}
where the first equality is from the definition of $\mathbf{H}_0$ in Eqn \eqref{def:neural_cbo_H0}, the first inequality is from the convexity of $\log \det(\cdot)$ function, and the second inequality is from the fact that $\langle \mathbf{A}, \mathbf{B} \rangle \le \norm{\mathbf{A}}_F \norm{\mathbf{B}}_F$. The third inequality is from the choice of $m$ in Condition \ref{condition:neural-cbo_network_width}, combined with Lemma \ref{lemma:neural_cbo_H_to_Phi} and Lemma 3 in \citet{chowdhury2017kernelized}.
