\chapter[Black-Box Optimization Algorithm Using Physics-Informed
...
\end{equation*}
where $\phi(\cdot)$ is the gradient of $h(\cdot; \boldsymbol{\theta}_0)$ with respect to the parameter $\boldsymbol{\theta}$, evaluated at initialization $\boldsymbol{\theta}_0$. Similarly, $\omega(\cdot)$ represents the gradient of $\mathcal{N}[h](\cdot; \boldsymbol{\theta}_0)$ with respect to model parameters $\boldsymbol{\theta}$ and evaluated at initialization $\boldsymbol{\theta}_0$, where $\mathcal{N}[h](\cdot; \boldsymbol{\theta}_0)$ is the result of applying differential operator $\mathcal{N}$ (with respect to the input) to $h(\cdot; \boldsymbol{\theta}_0)$. In this chapter, we focus on cases where the differential operator \( \mathcal{N} \), when applied to a \textit{real-valued} function \( h \), produces another \textit{real-valued} function \( \mathcal{N}[h] \) (e.g., linear differential operator with real constant coefficients). 
...
It is important to note that the physics information, expressed via the governing PDE, are incorporated into the loss function as additional penalty terms. These terms encourage the surrogate model not only to fit the observed data but also to satisfy the underlying physical laws of the problem domain. By penalizing deviations from the PDE constraints during training, the surrogate is guided to produce predictions that are consistent with established scientific knowledge, thereby improving both model fidelity and generalization in regions with limited data.
...
\begin{algorithm}[!ht]
\caption{Physics-informed Neural Network Black-box Optimization (PINN-BO)}
\label{alg:PINN-BO}
...
\State Update $ \boldsymbol{\theta}_t = \argmin_{\boldsymbol{\theta}} \mathcal{L}(\boldsymbol{\theta})$ using Eqn. \eqref{eqn:pinn_loss} by gradient descent with $\nu = \nu_t$.
\EndFor
\end{algorithmic}
\end{algorithm}
...
Generic \ac{bo} methods (without the extra information from \ac{pde}) utilize the \textit{maximum information gain} over search space $\mathcal{D}$ at time $t$:
...
\end{remark}
We are now ready to present the main theoretical result of this chapter: 
\begin{restatable}{theorem}{PINNBORegretBound}
\label{theorem:pinn-bo_main}
